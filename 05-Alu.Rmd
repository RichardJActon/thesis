...
Thus deterioration in the effectiveness of the repression of Alu elements has the potential to disruptively expose this latent regulatory function [@Ward2013; @Chuong2017; @Xie2013; @Zhang2019].
...
Details of the training and replication set to which elastic net regression was applied are detailed in [Sample Selection](#sampleSelection) \@ref(sampleSelection).
...
```{r modelSummaryTabs, echo = FALSE, warning=FALSE, message=FALSE}
# library(dplyr)
AluColours <- c(
	"AluY" = "#e41a1c",
	#"Y" = "#e41a1c",
...
selected_model_basic_summaries_split <-  readr::read_tsv(
	"figs/selected_model_basic_summaries_prop.tsv"
) %>% 
	dplyr::group_by(trainingSet, predictionSet) %>% 
	dplyr::group_split()

matched_mods <- selected_model_basic_summaries_split[c(2,3)]
matched_mods <- bind_rows(
	matched_mods[[1]] %>% mutate(group = "A"),
	matched_mods[[2]] %>% mutate(group = "B")
)

cross_mods <- selected_model_basic_summaries_split[c(1,4)]
cross_mods <- bind_rows(
	cross_mods[[1]] %>% mutate(group = "A"),
	cross_mods[[2]] %>% mutate(group = "B")
)

color_by_name <- function(
	val, name, colors, format = function(x){sprintf("%.3g", x)}
) {
	cell_spec(
		format(val),
		bold = TRUE,
		# color = ifelse(is.na(p_score), "#BBBBBB", "white"),
		background = colors[name]
	)
}

model_summary_tab_kable <- function(df, caption) {
	df %>%
	dplyr::mutate(
    name = gsub("%","\\%",name, fixed=TRUE),
		r = cell_spec(
			...
			sprintf("%.3f", r),
			bold = TRUE,
			color = ifelse(is.na(r), "#BBBBBB", "white"),
			background = case_when(
				#swsl == FALSE ~ "#BBBBBB",
				TRUE ~ spec_color(r, option = "C", end = 0.9) #E
			)
		),
		r2 = cell_spec(
			#format = "latex",
			sprintf("%.3f", r2),
			bold = TRUE,
			color = ifelse(is.na(r2), "#BBBBBB", "white"),
			background = case_when(
				#swsl == FALSE ~ "#BBBBBB",
				TRUE ~ spec_color(r2, option = "C", end = 0.9) #E
			)
		),
		across(
			.cols = c(NstartingWins, Nwins, NstartingAluElems, NAluElements),
			.fns = ~format(.x, big.mark = ",")
		),
		across(
			c(AluJ:FRAM),
			~color_by_name(
				.x, cur_column(), AluColours,
				format = function(x){sprintf("%.3g", 100 * x)}
			)
		)#,
		#pcNeg = ""
	) %>%
	dplyr::select(
			` ` = group,
			`Training Set` = trainingSet, `Prediction Set` = predictionSet,
			R = r, `R²` = r2, RMSE = rmse, MAE = mae,
			`Avail. Windows` = NstartingWins, `Clock Windows` = Nwins,
			`Avail. Alu Elems.` = NstartingAluElems,
			`Clock Alu Elems.` = NAluElements,
			Description = name, `\\% Coef. -ve` = pcNeg,
			#`Alu Type Proportions` = alu_stacked
			AluJ:FRAM
	) %>%
	kableExtra::kable(
...
```{r modelSummaryTabsMain, out.width='60%', echo=FALSE, warning=FALSE, message=FALSE}
model_summary_tab_kable(cross_mods,"(ref:modelSummaryTabsMain-Cap)")
```
(ref:modelSummaryTabsMain-Cap) __Alu Clock Model Summaries__ *A)* Models fitted with Training 1 set predicting Replication 1 ages. *B)* Models fitted with Training 2 set predicting Replication 2 ages. R is Pearson's correlation coefficient. RMSE = root mean squared Error, MAE = median absolute error.  '+' signifies this filter is in addition to element length >180bp and 99.9% non-zero RPM values at this locus.
...
```{r modelSummaryTabsCtrl, out.width='60%', echo=FALSE, warning=FALSE, message=FALSE}
...
model_summary_tab_kable(matched_mods, "(ref:modelSummaryTabsCtrl-Cap)")
#Alu Type Composition Colour code: `r alucolorkeytxt`.
```
(ref:modelSummaryTabsCtrl-Cap) __Summaries of Alu Clock Models Predicting Twin Groups__ for the purpose of comparing predictive of age models in related and unrelated samples. *A)* Models fitted with Training 1 set predicting Replication 2 ages. *B)* Models fitted with Training 2 set predicting Replication 1 ages. R is Pearson's correlation coefficient. RMSE = root mean squared Error, MAE = median absolute error.  '+' signifies this filter is in addition to element length >180bp and 99.9% non-zero RPM values at this locus.
...
These sites are listed in [appendix](#appAluageSNPs) \@ref(appAluageSNPs) with the top 10 sites from each GWAS shown in Figures \@ref(tab:m1tab) & \@ref(tab:m2tab) respectively. 
There is substantial overlap between the 2 sets with 143 unique SNPs in the combined list.

```{r, echo=FALSE, error=FALSE, warning=FALSE}
pretty_tab <- function(df, fileprefix) {
	df %>% mutate(
		locus = paste0("Chr", chr , ":", format(big.mark = ",", end))#,
		#`p-value` = sprintf("%.3g", p_score)
	) %>%
	select(locus, ref, alt, AF = af, p_score, SNP = rsid) %>%
	head(10) %>%
	dplyr::mutate(
		`p-value` = cell_spec(
			#format = "latex",
			...
			background = case_when(
				#swsl == FALSE ~ "#BBBBBB",
				TRUE ~ spec_color(p_score, option = "D", end = 0.9) #E
			)
		),
		AF = cell_spec(
			#format = "latex",
			sprintf("%.3f", AF),
			bold = TRUE,
			color = ifelse(is.na(AF), "#BBBBBB", "white"),
			background = case_when(
				#swsl == FALSE ~ "#BBBBBB",
				TRUE ~ spec_color(p_score, option = "C", end = 0.9) #E
			)
		)
	) %>%
	dplyr::select(-p_score) %>%
	kableExtra::kable(
		#format = "latex",
		#longtable = TRUE,
		caption = paste0("(ref:",gsub("_","",fileprefix),"-Cap)"),
	...
	#save_kable(paste0(fileprefix,".pdf"))

# magick::image_read_pdf(paste0(fileprefix,".pdf")) %>% 
# 	magick::image_convert("png") %>% 
# 	magick::image_write(paste0(fileprefix,".png"))
}

suggestivem1full <- readr::read_tsv("./figs/suggestivem1full.tsv")
suggestivem2full <- readr::read_tsv("./figs/suggestivem2full.tsv")
```
...
pretty_tab(suggestivem1full, "m1tab")
```
...
pretty_tab(suggestivem2full, "m2tab")
```
