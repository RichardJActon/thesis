...
Modelling work by @Affinito2016 suggests that once a threshold level of DNAm is reached in a region further methylation becomes less likely, meaning that sampling a single site is likely to produce a systematic underestimate.
...
```
(ref:Dedeurwaerder2011fig1bL-Cap) __Type I probes have a wider range and less variability than type II__. Type I probes shown in blue, Type II in red. Plot shows kernel density of beta values with gausian smoothing. (Reproduced from [@Dedeurwaerder2011])
...
This method was applied as a part of the pre-processing of the Twins UK 450k array datasets used in this work [Results 2](#tRNAs). 
...
Functional normalisation [@Fortin2014] which makes use of the technical control probe data on the arrays to inform its transformation of the DNA methylation values was used to normalise EPIC array data from the MAVIDOS and Southampton Women's Study (SWS) data in this work [Results 1](#arrays).
...
These masks have been used here to identify potentially problematic probes ([Results 1](#arrays), [Results 2](#tRNAs)).
...
MeDIP-seq uses a monoclonal anti-5mC antibody to bind denatured fragmented genomic DNA at methylated CpG sites, this antibody-bound fraction of DNA is isolated and sequenced [@Down2009], Figure \@ref(fig:MeDIPseqReaction) outlines the workflow.
...
(ref:Clark2012fig1-Cap) MeDIP-seq provides superior coverage of repetitive regions of the genome than the illumina array platform. MD-s = MeDIP-seq, 450k = Illumina 450k DNA methylation bead chip array (Reproduced from [@Clark2012])
...
```
(ref:MeDIPseqReaction-Cap) __Graphical Summary of the MeDIP-seq process__ Purified DNA is fragmented by sonication, denatured and incubated with anti-5mC antibodies. Before being immunoprecipitated resulting in fragments containing methylated CpGs which are then sequenced.
...
The above-described processing steps for Methylated DNA Immunoprecipitation sequencing (MeDIP-seq) data used here have been previously described by Bell et al. [@Bell2016; @Bell2017a].
...
Targeted Bisulfite sequencing very similar in principle to ordinary targeted DNA sequencing but preceded by a bisulfite conversion step as illustrated in Figure \@ref(fig:BisulfiteConversionNEB) [@Bock2010].
...
```
(ref:Krueger2011BismarkAlignment-Cap) __Bisulfite sequencing alignment process__  (Reproduced from [@Krueger2011])
