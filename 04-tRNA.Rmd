...
tRNA genes were clustered by grouping together all tRNAs within 5Mb of one another using the `bedtools merge` tool v2.17.0 [@Quinlan2010].
...
Using the EPIC array DNAm age and inferred using the Horvath clock [@Horvath2013] and blood cell-type-composition with the houseman method [@Houseman2012].
...
These array derived data were also used to estimate the major blood cell proportions for each of these pools with the Houseman algorithm [@Houseman2012].
...
I created customised MINTmap [@Loher2017] reference designed to include only fragments which unambiguously map to a single tRNA gene locus and which overlap the 5' or 3' end of the genomic tRNA sequence by at least one base with no mismatches.
...
The DNA methylation of the mouse tRNA genes was examined using data from a reduced representation bisulfite sequencing (RRBS) experiment performed by Petkovich et al. [@Petkovich2017].
...
As this result was observed in DNA from peripheral blood, a source with a heterogeneous cell-type population [@Lappalainen2017].
...
Codon usage frequencies have been claimed to be mostly invariant in the transcriptomes of a wide range of tissues, as well as across developmental time [@Schmitt2014b]. However, differences in the codon usage of genes with highly tissue-specific expression patterns have been observed [@Plotkin2004].
...
## Data availability {#tRNA-data-avail}
...
## Code availablity {#tRNA-code-avail}
